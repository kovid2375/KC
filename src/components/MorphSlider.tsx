...
export default function MorphSlider({
  ...
  return (
    ...
    >
      ...
      {showControls && (
        <div className="absolute top-1/2 left-0 right-0 z-30 flex justify-between px-5 -translate-y-1/2 pointer-events-none">
          <button
            type="button"
            className="pointer-events-auto inline-flex items-center justify-center w-12 h-12 rounded-full text-white border border-white/30 bg-black/60 backdrop-blur-md cursor-pointer transition-all duration-200 hover:scale-110 hover:bg-[#e52323] hover:border-transparent active:scale-95 shadow-2xl focus-visible:outline focus-visible:outline-2 focus-visible:outline-white/80"
      ...
      {showIndicators && (
        <div
          className="absolute left-0 right-0 top-6 z-30 flex gap-2 justify-center items-center pointer-events-auto"
          role="tablist"
          aria-label="Slides"
        >
          {items.map((item, i) => (
            <button
              ...
              className={`h-2 rounded-full cursor-pointer transition-all duration-300 focus-visible:outline focus-visible:outline-2 focus-visible:outline-white/80 ${
                i === index ? 'w-[28px] bg-[#e52323]' : 'w-2 bg-white/40 hover:bg-white/70'
              }`}
              onClick={() => {
